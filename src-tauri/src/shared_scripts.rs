...
use crate::models::{ScriptParamField, SharedScriptEntry, SharedScriptManifestScript};
...
fn list_manifest_scripts(
    ...
    let manifest: SharedScriptsManifest = serde_json::from_slice(&bytes).map_err(|error| {
        format!(
            "解析通用脚本清单失败（{}）：{}",
            manifest_path.display(),
            error
