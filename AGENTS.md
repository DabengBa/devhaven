...
- 终端头部快捷命令运行区（IDEA 风格固定在右上角：运行配置下拉 + 运行/停止）：`src/components/terminal/TerminalWorkspaceHeader.tsx`（UI） + `src/components/terminal/TerminalWorkspaceView.tsx`（配置选择与动作绑定） + `src/hooks/useQuickCommandRuntime.ts`（运行时状态机） + `src/hooks/useQuickCommandDispatch.ts`（外部派发）；运行/停止状态统一走 `src/services/quickCommands.ts` + `src-tauri/src/quick_command_manager.rs`；选中配置持久化写入 `terminal_workspaces.json` 的 `workspace.ui.runConfiguration.selectedScriptId`（类型：`src/models/terminal.ts`；默认/兼容处理：`src/utils/terminalLayout.ts`）
